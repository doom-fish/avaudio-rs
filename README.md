...
- The node, buffer, audio-unit, mixing and IO-node handle traits are sealed: only this crate's wrapper types implement them.
...
- `AudioSession` sample-rate, output-volume and other-audio queries return `AVAudioError::Unsupported`, because `AVAudioSession` is unavailable on macOS.
