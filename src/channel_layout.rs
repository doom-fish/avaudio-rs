...
impl AudioChannelLayout {
    ...
    pub fn new_with_layout_tag(layout_tag: AudioChannelLayoutTag) -> Result<Self, AVAudioError> {
        ...
        let ptr = unsafe {
            ffi::av_audio_channel_layout_create_with_layout_tag(layout_tag, &raw mut err)
        };
        if ptr.is_null() {
    ...
    pub fn info(&self) -> Result<AudioChannelLayoutInfo, AVAudioError> {
        let mut err: *mut c_char = ptr::null_mut();
        let json_ptr = unsafe { ffi::av_audio_channel_layout_info_json(self.ptr, &raw mut err) };
        if json_ptr.is_null() {
            return Err(unsafe { from_swift(ffi::status::OPERATION_FAILED, err) });
        }
