...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tempo_track.add_event(&MusicEvent::ExtendedTempo(ExtendedTempoEvent::new(120.0)), 0.0)?;

    let track = sequencer.create_and_append_track()?;
    track.add_event(&MusicEvent::MidiNote(MidiNoteEvent::new(0, 60, 96, 1.0)), 0.0)?;
    track.add_event(&MusicEvent::MusicUser(MusicUserEvent::new([1_u8, 2, 3, 4])), 2.0)?;
